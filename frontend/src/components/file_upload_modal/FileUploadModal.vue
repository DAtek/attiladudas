<template>
  <div class="modal is-active">
    ...
    <div class="modal-card">
      ...
      <form id="files-form" @submit="submit">
        <section class="modal-card-body has-text-left">
          <input type="file" multiple name="files" @change="setFiles"/>
          <p v-for="file of state.files" :key="file.name">
            <span>{{ file.name }}</span>
            <span class="help is-danger">{{ getErrorMessageForFile(file) }}</span>
          </p>
        </section>
        <footer class="modal-card-foot">
          <button type="submit" :disabled="state.uploadDisabled" class="button is-success">Upload files</button>
...
async function submit(event: Event) {
  ...
  result = await apiClient.postFiles({galleryId: fileUploadModalState.galleryId, formData})
  if (result.error) {
