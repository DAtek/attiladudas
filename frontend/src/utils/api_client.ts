...
class ApiClient {
    ...
    constructor(protected onError?: (e: unknown) => void) {
        ...
        this.putGalleryUrl = (id) => new URL(`/api/galleries/${id}/`, baseUrl())
        this.deleteGalleryUrl = (id) => new URL(`/api/galleries/${id}/`, baseUrl())
        this.postFilesUrl = (galleryId) => new URL(`/api/galleries/${galleryId}/files/`, baseUrl())
        this.deleteFilesUrl = (galleryId) => new URL(`/api/galleries/${galleryId}/files/`, baseUrl())
        this.patchFileRankUrl = (fileId: number, rank: number) => new URL(
    ...
    async postToken(data: PostTokenData): Promise<Result<null>> {
        ...
        if (response.status !== 201) return {
            error: {errors: [{location: "", type: "REQUIRED", context: null, message: ""}]},
            result: undefined
        }
    ...
    async postGallery(data: PostGalleryData): Promise<Result<PostGalleryResult>> {
        ...
        switch (response.status) {
            case 201:
                const result = JSON.parse(await response.text()) as PostGalleryResult
                return {error: undefined, result}
            case 400:
                const error = JSON.parse(await response.text()) as ApiError
...
function onError() {
     notificationCollection.addItem(new NotificationItem("DANGER", "Something unexpected happened. Check your network connection."))
}
